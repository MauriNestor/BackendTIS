...
const registrarPlanificacion = async (req, res) => {
  ...
  try {
    const result = await planificacionService.registrarPlanificacion(
      codigoClase,
      codigoGrupo
    );
    res.status(201).json({
...
const registrarRequerimientos = async (req, res) => {
  ...
  try {
    await planificacionService.registrarRequerimientos(
      codigoGrupo,
      requerimientos
    );
    res.status(201).json({
  ...
  try {
    if (!codSprint || !requerimientos || !Array.isArray(requerimientos)) {
      return res.status(400).json({
        message:
          "Datos incompletos. Asegúrate de proporcionar un codSprint y un arreglo de requerimientos.",
      });
    }
    await planificacionService.registrarRequerimientoASprint(
      codSprint,
      requerimientos
    );
...
const obtenerSprint = async (req, res) => {
  ...
  try {
    const sprintsConRequerimientos = await planificacionService.obtenerSprint(
      codigoGrupo
    );
...
const obtenerProductBacklog = async (req, res) => {
  ...
  try {
    const backlog = await planificacionService.obtenerProductBacklog(
      codigoGrupo
    );
...
const obtenerTodoProductBacklog = async (req, res) => {
  ...
  try {
    const backlog = await planificacionService.obtenerTodoProductBacklog(
      codigoGrupo
    );
    if (!backlog || backlog.length === 0) {
      return res.status(200).json([]); // Devuelve un arreglo vacío si no hay requerimientos
